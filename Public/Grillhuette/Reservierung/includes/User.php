        ...
        try {
            ...
            $this->logSecurityEvent(
                ...
                [
                    ...
                    'deleted_user_was_admin' => (bool) $userToDelete['is_admin']
                ]
            );
            return [
                'success' => true,
    ...
    public function updateUser($userId, $email, $firstName, $lastName, $phone, $isAdmin = 0, $newPassword = null, $isVerified = 1, $isAktivesMitglied = 0, $isFeuerwehr = 0)
    {
        try {
            ...
            if ($firstName !== $user['first_name'])
                $changes['first_name'] = ['old' => $user['first_name'], 'new' => $firstName];
            if ($lastName !== $user['last_name'])
                $changes['last_name'] = ['old' => $user['last_name'], 'new' => $lastName];
            if ($phone !== $user['phone'])
                $changes['phone'] = ['old' => $user['phone'], 'new' => $phone];
            if ($isAdmin != $user['is_admin'])
                $changes['is_admin'] = ['old' => (bool) $user['is_admin'], 'new' => (bool) $isAdmin];
            if ($isVerified != $user['is_verified'])
                $changes['is_verified'] = ['old' => (bool) $user['is_verified'], 'new' => (bool) $isVerified];
            if ($isAktivesMitglied != $user['is_AktivesMitglied'])
                $changes['is_AktivesMitglied'] = ['old' => (bool) $user['is_AktivesMitglied'], 'new' => (bool) $isAktivesMitglied];
            if ($isFeuerwehr != $user['is_Feuerwehr'])
                $changes['is_Feuerwehr'] = ['old' => (bool) $user['is_Feuerwehr'], 'new' => (bool) $isFeuerwehr];
