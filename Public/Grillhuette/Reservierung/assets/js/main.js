...
function setupMobileEnhancements() {
  ...
  if (isMobile) {
    ...
    document
      .querySelectorAll("a, button, .nav-link, .day")
      .forEach((element) => {
...
function initializeDatePickers() {
  ...
  if (datePickers.length > 0) {
    datePickers.forEach(function (picker) {
      flatpickr(picker, {
        ...
        appendTo: window.matchMedia("(max-width: 768px)").matches
          ? document.body
          : undefined,
      });
    });
  ...
  if (startDateInput && endDateInput) {
    const updateDisplayDateConstraints = function () {
      ...
      const endDate = endDateInput.value;
      const isDateRange =
        showDateRangeCheckbox && showDateRangeCheckbox.checked;
      if (startDate && endDate) {
        ...
        if (isDateRange) {
          ...
          if (displayStartInput.value) {
            ...
            if (
              displayStartDate < reservationStartDate ||
              displayStartDate > reservationEndDate
            ) {
          ...
          if (eventDayInput.value) {
            ...
            const reservationEndDate = new Date(endDate);
            if (
              eventDay < reservationStartDate ||
              eventDay > reservationEndDate
            ) {
...
function loadDayStatuses(month, year) {
  ...
  const rootPath =
    typeof APP_CONFIG !== "undefined" && APP_CONFIG.ROOT_PATH
  ...
  fetch(ajaxUrl)
    ...
    .then((text) => {
      try {
        ...
        if (!data.success) {
          handleError(
            data.message || "Unbekannter Fehler beim Laden der Daten"
          );
    ...
    .catch((error) => {
      ...
      if (calendarContainer) {
        ...
        errorElement.textContent =
          "Kalenderdaten konnten nicht geladen werden. Bitte versuchen Sie es später erneut.";
...
function updateDayStatuses(statusData) {
  ...
  document.querySelectorAll(".day[data-date]").forEach((dayElement) => {
    ...
    const existingIndicators = dayElement.querySelectorAll(
      ".event-indicator, .key-indicator"
    );
        ...
        if (typeof statusInfo === "object") {
          if (statusInfo.status === "public_event") {
            ...
            if (statusInfo.time_restrictions) {
              dayElement.dataset.timeRestrictions = JSON.stringify(
                statusInfo.time_restrictions
              );
...
function selectDay(dayElement) {
  ...
  if (startDateInput && endDateInput) {
    ...
    if (!startDateInput.value || (startDateInput.value && endDateInput.value)) {
      ...
      if (window.matchMedia("(max-width: 768px)").matches) {
        showMobileAlert(
          "Startdatum ausgewählt. Bitte wählen Sie jetzt das Enddatum."
        );
      }
      // Trigger cost update
      ...
      if (endDate < startDate) {
        ...
        if (window.matchMedia("(max-width: 768px)").matches) {
          showMobileAlert(
            "Daten wurden getauscht, da das Enddatum vor dem Startdatum lag."
          );
        }
      } else {
...
function highlightDateRange(startDate, endDate) {
  ...
  while (current <= end) {
    ...
    const dayElement = document.querySelector(
      `.day[data-date="${formattedDate}"]`
    );
    if (dayElement) {
      // Check if this day is selectable
      if (
        !isSelectable(dayElement) &&
  ...
  if (!allDaysInRangeAreSelectable) {
    const message =
      "Achtung: Der ausgewählte Zeitraum enthält Tage, die nicht verfügbar sind. Bitte wählen Sie einen anderen Zeitraum aus.";
    if (window.matchMedia("(max-width: 768px)").matches) {
      showMobileAlert(message);
    } else {
      alert(message);
  ...
  reservationForm.addEventListener("submit", function (event) {
    ...
    if (isPublicCheckbox && isPublicCheckbox.checked) {
      ...
      if (isDateRange && (!displayStartInput.value || !displayEndInput.value)) {
        event.preventDefault();
        const message =
          "Bitte wählen Sie Start- und Enddatum für den Veranstaltungszeitraum aus.";
        if (window.matchMedia("(max-width: 768px)").matches) {
          showMobileAlert(message);
        } else {
          alert(message);
      ...
      if (displayStart < reservationStart || displayEnd > reservationEnd) {
        event.preventDefault();
        const message =
          "Die Veranstaltungstage müssen innerhalb des Reservierungszeitraums liegen.";
        if (window.matchMedia("(max-width: 768px)").matches) {
          showMobileAlert(message);
        } else {
          alert(message);
    ...
    while (current <= end) {
      ...
      if (dayElement && !isSelectable(dayElement)) {
        ...
        event.preventDefault();
        const message =
          "Der ausgewählte Zeitraum enthält Tage, die nicht verfügbar sind. Bitte wählen Sie einen anderen Zeitraum aus.";
        if (window.matchMedia("(max-width: 768px)").matches) {
          showMobileAlert(message);
        } else {
          alert(message);
  ...
  if (startDateInput && endDateInput) {
    ...
    function setupFlatpickrEvents() {
      ...
      if (startDateInput._flatpickr) {
        startDateInput._flatpickr.config.onChange.push(function (
          selectedDates,
          dateStr
        ) {
      ...
      if (endDateInput._flatpickr) {
        endDateInput._flatpickr.config.onChange.push(function (
          selectedDates,
          dateStr
        ) {
          setTimeout(updateReservationCosts, 50);
...
function updateReservationCosts() {
  ...
  if (
    ...
    handleError(
      "Fehler bei der Preisberechnung: Nicht alle erforderlichen Elemente gefunden"
    );
    return;
  }
    ...
    try {
      ...
      fetch("Helper/get_pricing_info.php")
        ...
        .then((priceInfo) => {
          ...
          const rate =
            priceInfo.user_rate !== undefined && priceInfo.user_rate !== null
              ? priceInfo.user_rate
              : 100;
        ...
        .catch((error) => {
          ...
          if (costOverview && costOverview.hasAttribute("data-user-rate")) {
            const userRateVal = parseFloat(
              costOverview.getAttribute("data-user-rate")
            );
            defaultRate =
              !isNaN(userRateVal) && userRateVal !== null ? userRateVal : 100;
          }
          const totalCost = days * defaultRate;
          // Format for display with German notation
          const formattedRate = defaultRate.toLocaleString("de-DE", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          });
          const formattedTotal = totalCost.toLocaleString("de-DE", {
...
function calculateDefaultCosts(
  startDate,
  endDate,
  dayCountElement,
  totalCostElement,
...
function handleError(message) {
  ...
  if (calendarContainer) {
    ...
    errorElement.className = "alert alert-warning mt-3";
    errorElement.textContent =
      "Die Kalenderdaten konnten nicht geladen werden. Bitte versuchen Sie es später erneut.";
    calendarContainer.appendChild(errorElement);
